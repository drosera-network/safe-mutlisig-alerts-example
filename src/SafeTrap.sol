...
// Create a struct with any data you would like to use in the shouldAlert function
...
contract SafeTrap is Trap {
    ...
    address[] private _safeAddresses;

    constructor() {
          // TODO: Replace with the actual safe addresses
          address[] memory safeAddresses = new address[](1);
          safeAddresses[0] = address(0x0000000000000000000000000000000000000000);

          _safeAddresses = safeAddresses;
    }
    ...
    function shouldAlert(bytes[] calldata data) external pure override returns (bool, bytes memory) {
        CollectOutput memory currOutput = decodeAlertOutput(data[0]);

        // Check for unusual owner changes
        if (currOutput.totalOwnerChanges > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual threshold changes
        if (currOutput.totalThresholdChanges > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual module changes
        if (currOutput.totalModuleChanges > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual received changes
        if (currOutput.totalReceived > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual guard changes
        if (currOutput.totalGuardChanges > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual module executions
        if (currOutput.totalModuleExecutions > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual multi-sig transactions
        if (currOutput.totalMultiSigTransactions > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual module transactions
        if (currOutput.totalModuleTransactions > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual executions
        if (currOutput.totalExecutions > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual failures
        if (currOutput.totalFailures > 1) {
            return (true, abi.encode(currOutput));
        }

        // Check for unusual added owners
        if (currOutput.addedOwnerEvents.length > 1) {
            return (true, abi.encode(currOutput));
        }
        return (false, "");
    }

    function decodeAlertOutput(bytes memory data) public pure returns (CollectOutput memory) {
        return abi.decode(data, (CollectOutput));
    }

    function eventLogFilters() public view override returns (EventFilter[] memory) {
        EventFilter[] memory filters = new EventFilter[](_safeAddresses.length * 15);
        for (uint256 i = 0; i < _safeAddresses.length; i++) {
            address safe = _safeAddresses[i];

            // ExecutionSuccess(bytes32,uint256) 
            filters[i * 15] = EventFilter({contractAddress: safe, signature: "ExecutionSuccess(bytes32,uint256)"});

            // ExecutionFailure(bytes32,uint256)
            filters[i * 15 + 1] = EventFilter({contractAddress: safe, signature: "ExecutionFailure(bytes32,uint256)"});

            // AddedOwner(address)
            filters[i * 15 + 2] = EventFilter({contractAddress: safe, signature: "AddedOwner(address)"});

            // RemovedOwner(address)
            filters[i * 15 + 3] = EventFilter({contractAddress: safe, signature: "RemovedOwner(address)"});

            // ChangedThreshold(uint256)
            filters[i * 15 + 4] = EventFilter({contractAddress: safe, signature: "ChangedThreshold(uint256)"});

            // EnabledModule(address)
            filters[i * 15 + 5] = EventFilter({contractAddress: safe, signature: "EnabledModule(address)"});

            // DisabledModule(address)
            filters[i * 15 + 6] = EventFilter({contractAddress: safe, signature: "DisabledModule(address)"});

            // SafeSetup(address,address[],uint256,address,address)
            filters[i * 15 + 7] = EventFilter({contractAddress: safe, signature: "SafeSetup(address,address[],uint256,address,address)"});

            // SafeReceived(address,uint256)
            filters[i * 15 + 8] = EventFilter({contractAddress: safe, signature: "SafeReceived(address,uint256)"});

            // ChangedGuard(address)
            filters[i * 15 + 9] = EventFilter({contractAddress: safe, signature: "ChangedGuard(address)"});

            // ExecutionFromModuleSuccess(address)
            filters[i * 15 + 10] = EventFilter({contractAddress: safe, signature: "ExecutionFromModuleSuccess(address)"});

            // ExecutionFromModuleFailure(address)
            filters[i * 15 + 11] = EventFilter({contractAddress: safe, signature: "ExecutionFromModuleFailure(address)"});

            // SafeMultiSigTransaction(address,uint256,bytes,uint8,uint256,uint256,uint256,address,address,bytes,bytes)
            filters[i * 15 + 12] = EventFilter({contractAddress: safe, signature: "SafeMultiSigTransaction(address,uint256,bytes,uint8,uint256,uint256,uint256,address,address,bytes,bytes)"});

            // SafeModuleTransaction(address,address,uint256,bytes,uint8)
            filters[i * 15 + 13] = EventFilter({contractAddress: safe, signature: "SafeModuleTransaction(address,address,uint256,bytes,uint8)"});
        }
    ...
    function _getOwners(address _safe) internal view returns (address[] memory) {
        return ISafe(_safe).getOwners();
    }

    function _getThreshold(address _safe) internal view returns (uint256) {
        return ISafe(_safe).getThreshold();
    }

    function _getTransactionCount(address _safe) internal view returns (uint256) {
        return ISafe(_safe).getTransactionCount();
    }

    function _isOwner(address _safe, address owner) internal view returns (bool) {
        return ISafe(_safe).isOwner(owner);
    }

    function _getModules(address _safe) internal view returns (address[] memory) {
        return ISafe(_safe).getModules();
    }

    function _isModuleEnabled(address _safe, address module) internal view returns (bool) {
        return ISafe(_safe).isModuleEnabled(module);
    }
}
